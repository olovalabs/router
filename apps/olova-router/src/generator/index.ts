...
export function generateRouteTree(
  ...
  const routeImports = routes.map((route, index) => {
    ...
    if (route.hasMetadata || route.hasRoute) {
  ...
  const routeObjects = routes.map((route, index) => {
    const usesModule = route.hasMetadata || route.hasRoute;
    const component = usesModule ? "RouteModule" + index + ".default" : "Route" + index;
    const metadata = route.hasMetadata ? ", metadata: RouteModule" + index + ".metadata" : '';
    const routeDefinition = route.hasRoute ? ", routeDefinition: RouteModule" + index + ".route" : '';
    return "  { path: '" + route.path + "', component: " + component + metadata + routeDefinition + " }";
  }).join(',\n');
